...
def update_order_date_transport(request, order_id):
    if not Order.objects.filter(pk=order_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    order = Order.objects.get(pk=order_id)

    date_transport = request.data.get("date_transport")
    print(date_transport)
    if date_transport:
        order.date_transport = parse_datetime(date_transport)
        order.save()
        print(order.date_transport)
...
def update_status_user(request, order_id):
    ...
    calculate_date_transform(order_id)
...
def calculate_date_transform(order_id):
    data = {
        "order_id": order_id
    }

    requests.post("http://127.0.0.1:8080/calc_date_transport/", json=data, timeout=3)
